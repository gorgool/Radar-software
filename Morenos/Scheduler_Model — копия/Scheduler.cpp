...
std::int32_t Scheduler::find_index(const std::uint32_t transmit_len, const std::uint32_t receive_len, const std::uint32_t range_offset, const std::uint32_t depth)
{
  ...
  if (tr_length >= depth)
  {
    std::cerr << "ERROR: Transmit command length is larger than planning depth.\n";
    return -1;
  }
...
std::vector<ControlCommand> Scheduler::place_scan(const ScanPair & scan_pair, const std::uint32_t query_id)
{
  ...
  if (idx != -1)
  {
    ...
    std::uint64_t transmit_rephase_time = static_cast<std::uint64_t>(dcu_timeline.start_time + idx * settings::time_chunk_length);
    ...
    {
      std::cerr << "ERROR: range offset is not correct.\n";
      ...
      case QueryType::capture:
      {
        ...
        for (const auto& command : commands)
        {
          if (command.type != CommandType::ct_nop)
          {
            ...
            new_queries.push_back(std::make_shared<Query>(QueryType::tracking, q->id + 1, settings::tracking_query_speed, settings::tracking_query_threashold, time, q->range, q->rcs, true));
            break;
          }
        }
