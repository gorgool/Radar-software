...
static ScanPair create_scan_pair(const SignalParams& signal_params, const double tau, const double range, const QueryType type)
{
  assert(tau > 0.0);
  ...
  if (pulse_train_length > signal_params.pulse_number_high || pulse_train_length < signal_params.pulses_number_low)
  {
    ...
    std::cerr << "ERROR. Pulse train length is too high.\n";
    return ScanPair();
  }
  ...
  if (T_transmit > signal_params.period_high || T_transmit < signal_params.period_low)
  {
    ...
    std::cerr << "ERROR. Pulse train period is not valid.\n";
    return ScanPair();
  }
...
ScanPair build_scan_pair(const double range, const double rcs, const QueryType type)
{
  ...
  return create_scan_pair(signal_params, tau, range, type);
}