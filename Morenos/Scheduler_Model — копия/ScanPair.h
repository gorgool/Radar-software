...
public:
  ...
  ScanPair(const ScanPair& obj) : receive_time(obj.receive_time), size(obj.size)
  {
    receive = new receive_desc[size];
    transmit = new transmit_desc[size];
  ...
  {
    if (transmit[0].length == 0)
    {
      std::cerr << "ERROR: Can't set recieve offset time. Transmit duration is 0.";
      return;
    }
    ...
    if (size > 1)
    {
      ...
      {
        std::cerr << "ERROR: Pulse train transmit time is larger than range delay time offset.\n";
        return;
      }
    }
